...
if (isset($_POST["add_food_form"])) {
    ...
    if (move_uploaded_file($source, "../images/foods/" . $destination)) {
        $query = "INSERT INTO food VALUES(null, :title, :photo, :price, :type, :cat_id, :food_location, :food_cond, :brand)";
...
// Select Foods
$query_sel_food = "SELECT * FROM food";
$sel_food_prepare = $pdo->prepare($query_sel_food);
$sel_food_prepare->execute();
$food_num = $sel_food_prepare->rowCount();
$food_rows = $sel_food_prepare->fetchAll();
                        ...
                        <div class="panel panel-default">
                            <div class="panel-heading">
                                ...
                                <button class="btn btn-success" data-toggle="modal" data-target="#add">
                                    Add New Food <i class="fa fa-plus"></i>
                                </button>
                            </div> 
                            <div class="panel-body">
                                <?php
                                if ($food_num == 0) {
                                    echo "No Foods in the Database!";
                                } else {
                                ?>
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered table-hover">
                                        ...
                                        <tbody>
                                            <?php $x = 1; foreach ($food_rows as $food_row) { ?>
                                                <?php
                                                    $query = "SELECT * FROM category WHERE cat_id=".$food_row->cat_id;
                                                    $cat_id_stmt = $pdo->prepare($query);
                                                    $cat_id_stmt->execute();
                                                    $cat_row = $cat_id_stmt->fetch();
                                                ?>
                                                <tr>
                                                    <td><?= $x; ?></td>
                                                    <td><img src="../<?= $food_row->photo; ?>" style="width:60px;" class="img-responsive"/></td>
                                                    <td><?= $food_row->title; ?></td>
                                                    <td><?= $food_row->price; ?></td>
                                                    <td><?= $food_row->type; ?></td>
                                                    <td><?= $cat_row->title; ?></td>
                                                    <td><?= $food_row->food_location; ?></td>
                                                    <td><?= $food_row->food_cond; ?></td>
                                                    <td><?= $food_row->brand; ?></td>
                                                    <td><a href="update_food.php?id=<?= $food_row->food_id; ?>" class="btn btn-success">Update</a></td>
                                                    <td><a href="delete_food.php?id=<?= $food_row->food_id; ?>" class="btn btn-danger">Delete</a></td>
                                                </tr>
                                            <?php $x++; } ?>
                                        </tbody>
                                    </table>
