...
if (isset($_GET["id"])) {
	...
	$query_sel_food = "SELECT * FROM food WHERE food_id=:food_id";
	$sel_food_stmt = $pdo->prepare($query_sel_food);
	$sel_food_stmt->execute(["food_id"=>$id]);
	$food_row = $sel_food_stmt->fetch(PDO::FETCH_ASSOC);

	// Select the Category Title
	$query_sel_cat = "SELECT * FROM category WHERE cat_id=:cat_id";
	$sel_cat_stmt = $pdo->prepare($query_sel_cat);
	$sel_cat_stmt->execute(["cat_id"=>$food_row["cat_id"]]);
	$category = $sel_cat_stmt->fetch();

	// Select all food that belongs to current category
	$query_sel_foods = "SELECT * FROM food WHERE cat_id=:cat_id";
	$sel_foods_stmt = $pdo->prepare($query_sel_foods);
	$sel_foods_stmt->execute(["cat_id"=>$category->cat_id]);
	$food_rows = $sel_foods_stmt->fetchAll();
...
<div class="about" id="about">
	<div class="container">
		<div class="heading">
			<h3 data-aos="zoom-in"><?= $category->title; ?> (<?= $food_row["title"]; ?>)</h3>
		</div>
		<div class="row foods_category">
			<div data-aos="zoom-in" class="col-md-8">
				<div class="panel panel-default">
					<div class="panel-heading text-center"><?= $food_row["title"]; ?></div>
					<div class="panel-body">
						<img src="<?= $food_row['photo']; ?>" alt="">
						...
						<p style="text-align:left;">
							Location: <?= $food_row["food_location"]; ?>
						</p>
						<p style="text-align:left;">
							Condition: <?= $food_row["food_cond"]; ?>
						</p>
						<p style="text-align:left;">
							Brand: <?= $food_row["brand"]; ?>
						</p>
						<?php if (isset($_SESSION["logged_in"])) { ?>
							<a href="order.php?id=<?= $food_row['food_id']; ?>" class="btn btn-block btn-success" style="color:white;">Order Now <i class="fa fa-shopping-cart"></i></a>
						<?php } else { ?>
							<a href="login.php" class="btn btn-block btn-success" style="color:white;">Please Login First</a>
						<?php } ?>
			...
			<div data-aos="zoom-in" class="col-md-4">
				<div class="panel panel-default">
					<div class="panel-heading text-center"><?= $category->title; ?></div>
					<div class="panel-body">
						<?php foreach ($food_rows as $food_row) { ?>
							<p style="text-align:left;">
								<a href="food_details.php?id=<?= $food_row->food_id; ?>"><?= $food_row->title; ?> <i class="fa fa-shopping-cart" style="float:right;"></i></a>
							</p>
						<?php } ?>
					</div>
				</div>
