...
// show the alert if the customer successfully logout
if (isset($_GET["logoutSuccessfully"])) {
	?>
		<script>
			alert("You Successfully Logouted!");
...
// All Category
$query_sel_cats = "SELECT * FROM category";
$cats_stmt = $pdo->prepare($query_sel_cats);
$cats_stmt->execute();
$cat_rows = $cats_stmt->fetchAll();
				...
				<div class="container">
					...
					<div class="collapse navbar-collapse navbar-ex1-collapse">
						<ul class="nav navbar-nav navbar-right">
							...
							<li><a class="page-scroll scroll" href="#about">Category</a></li>
...
<div class="about" id="about">
	<div class="container">
		<div class="heading">
			<h2 data-aos="zoom-in">Foods Category</h2>
		</div>
		<div class="row foods_category">
			<?php foreach ($cat_rows as $cat_row) { ?>
				<?php
					$query_sel_foods = "SELECT * FROM food WHERE cat_id=:cat_id";
					$foods_stmt = $pdo->prepare($query_sel_foods);
					$foods_stmt->execute(["cat_id"=>$cat_row->cat_id]);
					$foods_num = $foods_stmt->rowCount();
					$food_rows = $foods_stmt->fetchAll();
				?>
				<div data-aos="zoom-in" class="col-md-4" style="min-height:160px;">
					<div class="panel panel-default">
						<div class="panel-heading text-center"><?= $cat_row->title; ?></div>
						<div class="panel-body">
							<?php if ($foods_num != 0) { ?>
								...
								<?php } ?>
							<?php } else echo "This Category has not any Food!"; ?>
						</div>
					</div>
				</div>
			<?php } ?>
